...
class MarginCategories extends Model
{
    use HasFactory;
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($query) use ($search) {
                foreach ($this->filterable as $field) {
                    ...
                    elseif ($field === 'sub_classifications_id')  {
                        $query->orWhereHas('getSubClassification', function ($query) use ($search) {
                            $query->where('subclass_description', 'LIKE', "%$search%");
                        });
                    } 

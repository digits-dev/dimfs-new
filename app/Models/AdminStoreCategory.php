...
class AdminStoreCategory extends Model
{
    use HasFactory;
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($query) use ($search) {
                foreach ($this->filterable as $field) {
                    ...
                    elseif ($field === 'admin_sub_classifications_id')  {
                        $query->orWhereHas('getAdminSubClassification', function ($query) use ($search) {
                            $query->where('sub_class_description', 'LIKE', "%$search%");
                        });
                    }

...
class RmaClassifications extends Model
{
    use HasFactory;

    protected $table = 'rma_classifications';
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($query) use ($search) {
                foreach ($this->filterable as $field) {
                    ...
                    elseif ($field === 'rma_categories_id')  {
                        $query->orWhereHas('getRmaCategory', function ($query) use ($search) {
    ...
    public function getRmaCategory() {
        return $this->belongsTo(RmaCategories::class, 'rma_categories_id', 'id');
    }

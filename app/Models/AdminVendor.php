        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($query) use ($search) {
                foreach ($this->filterable as $field) {
                    ...
                    elseif ($field === 'admin_brands_id')  {
                        $query->orWhereHas('getAdminBrand', function ($query) use ($search) {
                            $query->where('brand_description', 'LIKE', "%$search%");
                        });
                    }
                    elseif ($field === 'admin_incoterms_id')  {
                        $query->orWhereHas('getAdminIncoterm', function ($query) use ($search) {

...
const GashaponVendorGroupsAction = ({action, onClose, updateData}) => {
    ...
    const { data, setData, processing, reset, post, errors } = useForm({
        id: "" || updateData.id,
        vendor_group_description: "" || updateData.vendor_group_description,
    ...
    const handleFormSubmit = (e) => {
        e.preventDefault();
        Swal.fire({
            title: `<p class="font-poppins text-3xl" >Do you want ${action == 'Add' ? 'add' : 'update'} Gashapon Vendor Group?</p>`,
            ...
            if (result.isConfirmed) {
                ...
                if (action == 'Add'){
                    post('gashapon_vendor_groups/create', {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["gashapon_vendor_groups"] });
                            reset();
                            onClose();
                        },
                        onError: (error) => {
                    ...
                    post('gashapon_vendor_groups/update', {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["gashapon_vendor_groups"] });
        ...
        {action !== 'View' && 
            <div className='flex justify-end'>
                ...
                {processing ? 
                    ...
                    (
                        <span>
                            <i className={`fa-solid ${action === "Add" ? 'fa-plus' : 'fa-pen-to-square' } mr-1`}></i> {action === "Add" ? 'Add Gashapon Vendor Group' : 'Update Gashapon Vendor Group'}
                        </span>
                    )
                }
                </Button>  
...
export default GashaponVendorGroupsAction
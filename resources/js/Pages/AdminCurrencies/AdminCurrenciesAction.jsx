...
const AdminCurrenciesAction = ({action, onClose, updateData}) => {
    ...
    const { data, setData, processing, reset, post, errors } = useForm({
        ...
        currency_description: "" || updateData.currency_description,
    ...
    const handleFormSubmit = (e) => {
        e.preventDefault();
        Swal.fire({
            title: `<p class="font-poppins text-3xl" >Do you want ${action == 'Add' ? 'add' : 'update'} Currency?</p>`,
            ...
            if (result.isConfirmed) {
                ...
                if (action == 'Add'){
                    post('admin_currencies/create', {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["admin_currencies"] });
                            reset();
                            onClose();
                        },
                        onError: (error) => {
  ...
  return (
    <form onSubmit={handleFormSubmit} className='space-y-2'>
        <InputComponent
            ...
            onError={errors.currency_code}
        />
        <InputComponent
            name="currency_description"
            value={data.currency_description}
            disabled={action === 'View'}
            placeholder="Enter Currency Description"
            onChange={(e)=> setData("currency_description", e.target.value.toUpperCase())}
            onError={errors.currency_description}
        />
        ...
        {action !== 'View' && 
            <div className='flex justify-end'>
                ...
                {processing ? 
                    ...
                    (
                        <span>
                            <i className={`fa-solid ${action === "Add" ? 'fa-plus' : 'fa-pen-to-square' } mr-1`}></i> {action === "Add" ? 'Add Currency' : 'Update Currency'}
                        </span>
                    )
                }
                </Button>  
...
export default AdminCurrenciesAction
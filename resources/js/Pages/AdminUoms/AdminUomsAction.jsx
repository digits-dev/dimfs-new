...
const AdminUomsAction = ({action, onClose, updateData}) => {
    ...
    const handleFormSubmit = (e) => {
        e.preventDefault();
        Swal.fire({
            title: `<p class="font-poppins text-3xl" >Do you want ${action == 'Add' ? 'add' : 'update'} UOM?</p>`,
            ...
            if (result.isConfirmed) {
                ...
                if (action == 'Add'){
                    post('admin_uoms/create', {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["admin_uoms"] });
                            reset();
                            onClose();
                        },
                        onError: (error) => {
  ...
  return (
    <form onSubmit={handleFormSubmit} className='space-y-2'>
        ...
        <InputComponent
            ...
            onError={errors.uom_description}
        />
        ...
        {action !== 'View' && 
            <div className='flex justify-end'>
                ...
                {processing ? 
                    ...
                    (
                        <span>
                            <i className={`fa-solid ${action === "Add" ? 'fa-plus' : 'fa-pen-to-square' } mr-1`}></i> {action === "Add" ? 'Add UOM' : 'Update UOM'}
                        </span>
                    )
                }
                </Button>  
...
export default AdminUomsAction
...
const ForceChangePassword = () => {
    ...
    const handleSubmit = async (e, action) => {
        ...
        // Only proceed with the action if it is 'change'
        if (action === 'change') {
            ...
            // Waive password change logic
            if (!checkCountWaive) {
                await checkWaiveCount();
            }
            setLoading(true);
            try {
                const response = await axios.post('/waive-change-password', {
                    headers: {
                ...
                Swal.fire({
                    type: 'error',
                    title: 'An error occurred while waiving the password',
                    icon: 'error',
                    confirmButtonColor: swalColor,
                });
            } finally {
    ...
    const checkWaiveCount = async () => {
        try {
            const response = await axios.post('/check-waive',{
                headers: {
                    'Content-Type': 'application/json',
            ...
            if (response.data.status) {
                ...
                setCheckCountWaive(true);
            }
        } catch (error) {
            setCheckCountWaive(false);
            Swal.fire({ 
                type: 'error',
                title: 'An error occurred while verifying waive count',
                icon: 'error',
                confirmButtonColor: swalColor,
            });
        }
    ...
    return (
        <>
            ...
            <ChangePassModal
                ...
                isPasswordQwerty={isPasswordQwerty}
                loading={loading}
            >
                <form className="p-2">

...
const ItemMastersFilter = ({filter_inputs}) => {
            ...
            {filter_inputs.map((input, index) => (
                <div key={index} className="w-full">
                    <MultiTypeInput
                        name={input.name}
                        type={input.type}
                        onChange={(selectedValue) => handleFilter(selectedValue, input.name, input.type)}
                        displayName={input.header_name}
                        placeholder={`Enter ${input.header_name}`}
                        menuPlacement={index === filter_inputs.length - 1 ? "top" : "auto"}
...
export default ItemMastersFilter
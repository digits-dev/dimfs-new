...
const AdminVendorTypesAction = ({action, onClose, updateData}) => {
    ...
    const handleFormSubmit = (e) => {
        e.preventDefault();
        Swal.fire({
            title: `<p class="font-poppins text-3xl" >Do you want ${action == 'Add' ? 'add' : 'update'} Vendor Type?</p>`,
            ...
            if (result.isConfirmed) {
                ...
                if (action == 'Add'){
                    post('admin_vendor_types/create', {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["admin_vendor_types"] });
                            reset();
                            onClose();
                        },
                        onError: (error) => {
                    ...
                    post('admin_vendor_types/update', {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["admin_vendor_types"] });
  ...
  return (
    <form onSubmit={handleFormSubmit} className='space-y-2'>
        <InputComponent
            ...
            onError={errors.vendor_type_code}
        />
        <InputComponent
            ...
            onError={errors.vendor_type_description}
        />
        ...
        {action !== 'View' && 
            <div className='flex justify-end'>
                ...
                {processing ? 
                    ...
                    (
                        <span>
                            <i className={`fa-solid ${action === "Add" ? 'fa-plus' : 'fa-pen-to-square' } mr-1`}></i> {action === "Add" ? 'Add Vendor Type' : 'Update Vendor Type'}
                        </span>
                    )
                }
                </Button>  
...
export default AdminVendorTypesAction
...
const BrandGroupsAction = ({action, onClose, updateData}) => {
    ...
    const { data, setData, processing, reset, post, errors } = useForm({
        id: "" || updateData.id,
        brand_group_description: "" || updateData.brand_group_description,
    ...
    const handleFormSubmit = (e) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                if (action == 'Add'){
                    post('brand_groups/create', {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["brand_groups"] });
                            reset();
                            onClose();
                        },
                        onError: (error) => {
        ...
        {action !== 'View' && 
            <div className='flex justify-end'>
                ...
                {processing ? 
                    ...
                    (
                        <span>
                            <i className={`fa-solid ${action === "Add" ? 'fa-plus' : 'fa-pen-to-square' } mr-1`}></i> {action === "Add" ? 'Add Brand Group' : 'Update Brand Group'}
                        </span>
                    )
                }
                </Button>  
...
export default BrandGroupsAction
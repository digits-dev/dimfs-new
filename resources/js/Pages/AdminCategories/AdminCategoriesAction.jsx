...
const AdminCategoriesAction = ({action, onClose, updateData}) => {
    ...
    const handleFormSubmit = (e) => {
        e.preventDefault();
        Swal.fire({
            title: `<p class="font-poppins text-3xl" >Do you want ${action == 'Add' ? 'add' : 'update'} Category?</p>`,
                ...
                if (action == 'Add'){
                    post('admin_categories/create', {
                        onSuccess: (data) => {
                            ...
                            router.reload({ only: ["admin_categories"] });
                            reset();
                            onClose();
                        },
                        onError: (error) => {
  ...
  return (
    <form onSubmit={handleFormSubmit} className='space-y-2'>
        ...
        <InputComponent
            ...
            onError={errors.category_description}
        />
        ...
        {action !== 'View' && 
            <div className='flex justify-end'>
                ...
                {processing ? 
                    ...
                    (
                        <span>
                            <i className={`fa-solid ${action === "Add" ? 'fa-plus' : 'fa-pen-to-square' } mr-1`}></i> {action === "Add" ? 'Add Category' : 'Update Category'}
                        </span>
                    )
                }
                </Button>  

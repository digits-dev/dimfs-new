...
const ItemSegmentationsAction = ({ action, onClose, updateData }) => {
    ...
    const handleFormSubmit = (e) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                if (action == "Add") {
                    post("item_segmentations/create", {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["item_segmentations"] });
                            reset();
                            onClose();
                        },
                        onError: (error) => {},
                    ...
                    post("item_segmentations/update", {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["item_segmentations"] });
...
export default ItemSegmentationsAction;

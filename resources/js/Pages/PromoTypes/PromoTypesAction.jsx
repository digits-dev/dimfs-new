...
const PromoTypesAction = ({ action, onClose, updateData }) => {
    ...
    const { data, setData, processing, reset, post, errors } = useForm({
        id: "" || updateData.id,
        promo_type_column: "" || updateData.promo_type_column,
        promo_type_description: "" || updateData.promo_type_description,
    ...
    const handleFormSubmit = (e) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                if (action == "Add") {
                    post("promo_types/create", {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["promo_types"] });
                            reset();
                            onClose();
                        },
                        onError: (error) => {},
    ...
    return (
        <form onSubmit={handleFormSubmit} className="space-y-2">
            ...
            <InputComponent
                ...
                value={data.promo_type_column}
                disabled={action === "View"}
                placeholder="Enter Promo Type Column"
                onChange={(e) => setData("promo_type_column", e.target.value)}
            />
            {errors.promo_type_column && (
            ...
            <InputComponent
                ...
                value={data.promo_type_description}
                disabled={action === "View"}
...
export default PromoTypesAction;

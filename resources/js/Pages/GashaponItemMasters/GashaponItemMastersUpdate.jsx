...
const GashaponItemMastersUpdate = ({page_title, update_inputs, gashapon_item_master_detail}) => {
  const { theme } = useTheme();
  const { handleToast } = useToast();
  const { primayActiveColor, textColorActive, buttonSwalColor } = useThemeStyles(theme);

  const initialFormData = update_inputs.reduce((acc, item) => {
    acc[item.name] = "" || gashapon_item_master_detail[item.name];
    return acc;
  }, { validation: "", id: gashapon_item_master_detail.id });

  const { data, setData, processing, reset, post, errors } = useForm(initialFormData);

  // APPLYING VALIDATION
  useEffect(()=>{
    console.log(gashapon_item_master_detail, update_inputs);
  ...
  const handleFormSubmit = (e) => {
      e.preventDefault();
      Swal.fire({
          title: `<p class="font-poppins text-3xl" >Do you want to update Gashapon Item?</p>`,
          ...
          if (result.isConfirmed) {
              post("/gashapon_item_masters/update", {
                  onSuccess: (data) => {
  ...
  return (
    <>
      ...
      <ContentPanel>
        <p className="text-lg font-semibold mb-2">Update Gashapon Item Master Data</p>
            ...
            <form onSubmit={handleFormSubmit}>
                ...
                {update_inputs.map((input, index) => (
                  <div key={index} className="w-full">
                    <MultiTypeInput
                      name={input.name}
                      type={input.type}
                      value={input.type == 'select' ? { label: input.table_join.split('.').reduce((acc, key) => acc?.[key], gashapon_item_master_detail), value: gashapon_item_master_detail[input.name] }  
                          : data[input.name]
                      }
              ...
              <div className="flex justify-between mt-4">
                  ...
                  >
                      ...
                      (
                          <span><i className={`fa-solid fa-edit mr-1`}></i>{" "}Update Gashapon Item</span>
                      )}
                  </Button>
              </div>
            </form>
...
export default GashaponItemMastersUpdate
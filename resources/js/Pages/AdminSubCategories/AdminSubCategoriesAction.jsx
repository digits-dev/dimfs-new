...
const AdminSubCategoriesAction = ({action, onClose, updateData, all_active_admin_categories, all_admin_categories}) => {
    ...
    const { data, setData, processing, reset, post, errors } = useForm({
        id: "" || updateData.id,
        sub_category_code: "" || updateData.sub_category_code,
        sub_category_description: "" || updateData.sub_category_description,
    ...
    const handleFormSubmit = (e) => {
        e.preventDefault();
        Swal.fire({
            title: `<p class="font-poppins text-3xl" >Do you want ${action == 'Add' ? 'add' : 'update'} Sub Category?</p>`,
            ...
            if (result.isConfirmed) {
                ...
                if (action == 'Add'){
                    post('admin_sub_categories/create', {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["admin_sub_categories"] });
                            reset();
                            onClose();
                        },
                        onError: (error) => {
                    ...
                    post('admin_sub_categories/update', {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["admin_sub_categories"] });
  ...
  return (
    <form onSubmit={handleFormSubmit} className='space-y-2'>
        <InputComponent
            ...
            onChange={(e)=> setData("sub_category_code", e.target.value.toUpperCase())}
            onError={errors.sub_category_code}
        />
        <InputComponent
            name="sub_category_description"
            value={data.sub_category_description}
            disabled={action === 'View'}
            displayName="Sub Category Description"
            placeholder="Enter Sub Category Description"
            onChange={(e)=> setData("sub_category_description", e.target.value.toUpperCase())}
            onError={errors.sub_category_description}
        />
        {action == 'View' && 
            <InputComponent
        ...
        {action !== 'View' && 
            <div className='flex justify-end'>
                ...
                {processing ? 
                    ...
                    (
                        <span>
                            <i className={`fa-solid ${action === "Add" ? 'fa-plus' : 'fa-pen-to-square' } mr-1`}></i> {action === "Add" ? 'Add Sub Category' : 'Update Sub Category'}
                        </span>
                    )
                }
                </Button>  
...
export default AdminSubCategoriesAction
...
const RmaCategoriesAction = ({action, onClose, updateData}) => {
    ...
    const handleFormSubmit = (e) => {
        e.preventDefault();
        Swal.fire({
            title: `<p class="font-poppins text-3xl" >Do you want ${action == 'Add' ? 'add' : 'update'} RMA Category?</p>`,
            ...
            if (result.isConfirmed) {
                ...
                if (action == 'Add'){
                    post('rma_categories/create', {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["rma_categories"] });
                            reset();
                            onClose();
                        },
                        onError: (error) => {
  ...
  return (
    <form onSubmit={handleFormSubmit} className='space-y-2'>
        ...
        <InputComponent
            ...
            placeholder="Enter RMA Category Code"
        ...
        <InputComponent
            ...
            placeholder="Enter RMA Category Description"
        ...
        {action !== 'View' && 
            <div className='flex justify-end'>
                ...
                {processing ? 
                    ...
                    (
                        <span>
                            <i className={`fa-solid ${action === "Add" ? 'fa-plus' : 'fa-pen-to-square' } mr-1`}></i> {action === "Add" ? 'Add RMA Category' : 'Update RMA Category'}
                        </span>
                    )
                }
                </Button>  
...
export default RmaCategoriesAction
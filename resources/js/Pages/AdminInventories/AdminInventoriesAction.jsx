...
const AdminInventoriesAction = ({action, onClose, updateData}) => {
    ...
    const handleFormSubmit = (e) => {
        e.preventDefault();
        Swal.fire({
            title: `<p class="font-poppins text-3xl" >Do you want ${action == 'Add' ? 'add' : 'update'} Inventory?</p>`,
            ...
            if (result.isConfirmed) {
                ...
                if (action == 'Add'){
                    post('admin_inventories/create', {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["admin_inventories"] });
                            reset();
                            onClose();
                        },
                        onError: (error) => {
  ...
  return (
    <form onSubmit={handleFormSubmit} className='space-y-2'>
        <InputComponent
            ...
            onChange={(e)=> setData("inventory_code", e.target.value.toUpperCase())}
            onError={errors.inventory_code}
        />
        <InputComponent
            ...
            onChange={(e)=> setData("inventory_description", e.target.value.toUpperCase())}
            onError={errors.inventory_description}
        />
        ...
        {action !== 'View' && 
            <div className='flex justify-end'>
                ...
                {processing ? 
                    ...
                    (
                        <span>
                            <i className={`fa-solid ${action === "Add" ? 'fa-plus' : 'fa-pen-to-square' } mr-1`}></i> {action === "Add" ? 'Add Inventory' : 'Update Inventory'}
                        </span>
                    )
                }
                </Button>  
...
export default AdminInventoriesAction
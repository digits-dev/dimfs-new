...
const AdminModelSpecificsAction = ({action, onClose, updateData}) => {
    ...
    const handleFormSubmit = (e) => {
        e.preventDefault();
        Swal.fire({
            title: `<p class="font-poppins text-3xl" >Do you want ${action == 'Add' ? 'add' : 'update'} Model Specific?</p>`,
            ...
            if (result.isConfirmed) {
                ...
                if (action == 'Add'){
                    post('admin_model_specifics/create', {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["admin_model_specifics"] });
                            reset();
                            onClose();
                        },
                        onError: (error) => {
                    ...
                    post('admin_model_specifics/update', {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["admin_model_specifics"] });
  ...
  return (
    <form onSubmit={handleFormSubmit} className='space-y-2'>
        ...
        <InputComponent
            ...
            onError={errors.model_specific_code}
        />
        {/* BRAND DESCRIPTION */}
        <InputComponent
            ...
            onError={errors.model_specific_description}
        />
        ...
        {action !== 'View' && 
            <div className='flex justify-end'>
                ...
                {processing ? 
                    ...
                    (
                        <span>
                            <i className={`fa-solid ${action === "Add" ? 'fa-plus' : 'fa-pen-to-square' } mr-1`}></i> {action === "Add" ? 'Add Model Specific' : 'Update Model Specific'}
                        </span>
                    )
                }
                </Button>  
...
export default AdminModelSpecificsAction
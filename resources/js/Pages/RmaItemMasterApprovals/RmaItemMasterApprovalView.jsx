...
const RmaItemMasterApprovalView = ({
    page_title,
    rma_item_master_approval,
    table_headers,
    action,
}) => {
    ...
    const { data, setData, processing, reset, post, errors } = useForm({
        id: rma_item_master_approval.id,
        action: "",
    });
    ...
    const handleFormSubmit = (e) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                post("/rma_item_master_approvals/approval", {
                    onSuccess: (data) => {
                ...
                <div className="border p-4 rounded-lg">
                    <form onSubmit={handleFormSubmit}>
                        <div className="md:grid md:grid-cols-2 md:gap-2 space-y-2 md:space-y-0">
                            {table_headers?.map((header, index) => (
                                <InputComponent
                                    ...
                                    value={
                                        rma_item_master_approval?.item_values?.[
                                            header.name
                                        ] ?? ""
                                    }
                                    disabled={true}
                        ...
                        <div className="flex justify-between mt-4">
                            <Button
                                type="link"
                                href="/rma_item_master_approvals"
                                extendClass={`${
                                    theme === "bg-skin-white"
                                        ? primayActiveColor
                                        : theme
...
export default RmaItemMasterApprovalView;

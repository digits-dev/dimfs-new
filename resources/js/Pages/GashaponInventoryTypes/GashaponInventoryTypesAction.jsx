...
const GashaponInventoryTypesAction = ({action, onClose, updateData}) => {
    ...
    const handleFormSubmit = (e) => {
        e.preventDefault();
        Swal.fire({
            title: `<p class="font-poppins text-3xl" >Do you want ${action == 'Add' ? 'add' : 'update'} Gashapon Inventory Type?</p>`,
            ...
            if (result.isConfirmed) {
                ...
                if (action == 'Add'){
                    post('gashapon_inventory_types/create', {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["gashapon_inventory_types"] });
                            reset();
                            onClose();
                        },
                        onError: (error) => {
                    ...
                    post('gashapon_inventory_types/update', {
                        onSuccess: (data) => {
                            const { message, type } = data.props.auth.sessions;
                            handleToast(message, type);
                            router.reload({ only: ["gashapon_inventory_types"] });
  ...
  return (
    <form onSubmit={handleFormSubmit} className='space-y-2'>
        ...
        <InputComponent
            ...
            placeholder="Enter Gashapon Inventory Type Description"
            onChange={(e)=> setData("inventory_type_description", e.target.value.toUpperCase())}
        />
        ...
        {action !== 'View' && 
            <div className='flex justify-end'>
                ...
                {processing ? 
                    ...
                    (
                        <span>
                            <i className={`fa-solid ${action === "Add" ? 'fa-plus' : 'fa-pen-to-square' } mr-1`}></i> {action === "Add" ? 'Add Gashapon Inventory Type' : 'Update Gashapon Inventory Type'}
                        </span>
                    )
                }
                </Button>  
...
export default GashaponInventoryTypesAction
...
const ApprovalBulkActions = ({disabled, setData, onConfirm}) => {
  ...
  return (
    <>
        <div ref={notifRef} className={`z-[60] select-none relative inline-block text-left font-poppins ${disabled ? 'opacity-50' : 'opacity-100' }`}>
            ...
            >
                <div className={`flex items-center gap-3 px-3 py-2 cursor-pointer ${theme === 'bg-skin-black' ? 'hover:bg-black/20' : 'hover:bg-gray-200'}`} onClick={()=>{handleMenuToggle(); handleModalToggle(); setBulkAction('Approve'); setData('bulkAction', 'Approve');}}>
                    <i className={`fa-solid fa-circle-check text-green-500 text-wrap text-xs md:text-base`}></i>
                    <p className='text-[10px] md:text-xs font-medium text-nowrap'>Approve Item/s</p>
                </div>
                <div className={`flex items-center gap-3 px-3 py-2 cursor-pointer ${theme === 'bg-skin-black' ? 'hover:bg-black/20' : 'hover:bg-gray-200'}`} onClick={()=>{handleMenuToggle(); handleModalToggle(); setBulkAction('Reject'); setData('bulkAction', 'Reject');}}>
                    <i className={`fa-solid fa-circle-xmark text-red-500 text-wrap text-xs md:text-base`}></i>
                    <p className='text-[10px] md:text-xs font-medium '>Reject Item/s</p>
                </div>
        ...
        <div className={`${theme === 'bg-skin-black' ? 'bg-black-table-color text-gray-300' : 'bg-white'} p-5 max-w-[30rem] w-full m-5 rounded-lg shadow-lg duration-300 ease-in-out ${isModalOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
            ...
            <>
                ...
                <p className={`${theme == 'bg-skin-black' ? 'text-white' : 'text-black'} text-sm text-gray-500 mb-7`}>{bulkAction} selected items?</p>
